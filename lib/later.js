            ...
            */
            getNext: function (recur, startDate, endDate) {
                var schedules = recur ? recur.schedules || [] : [],
                    ...
                    hasExceptions = exceptions.schedules.length > 0,
                ...
                while(!done && start) {
                    ...
                    if (i === 0) {  // no constraints, start time is fine
                        ...
                        while(i--) {
                            ...
                            if (!date || (tDate.getTime() < date.getTime())) {
                                date = tDate;
                            }
                        }
                    }

                    if (date && hasExceptions) {
                        tDate = this.getNextInvalid(exceptions, date);
            ...
            */
            getNextInvalid: function (recur, startDate) {
                var schedules = recur ? recur.schedules || [] : [],
                    ...
                    start = startDate || new Date(),
                    tDate, nextExcep, nextInvalid,
                    done = false, i;

                while(start && this.isValid(recur, start)) {
                    nextExcep = nextInvalid = null;

                    // get the next invalid schedule if there is one
                    i = schedules.length;
                    while (i--) {
                        tDate = getNextInvalidSchedule(schedules[i], start);
                        if (!nextInvalid || (tDate.getTime() > nextInvalid.getTime())) {
                            nextInvalid = tDate;
                        }
                    }
