...
(function() {
	...
	var Scheduler = function(resolution) {
		...
		var _date = function(date) {
			...
			var yr, dfY, dlY, wOY, mt, dOY, ldY, dOM, dOW, hr, mn, sc, ti;

			var MS_PER_DAY = 86400000;
			var DAYS_PER_WEEK = 7

			var clear = function() {
				yr = dfY = dlY = wOY = mt = dOY = ldY = dOM = dOW = hr = mn = sc = ti = null;
			}
			...
			return {
				yr: 	yr  || (yr = date.getUTCFullYear()),
				dfY: 	dfY || (dfY = new Date(Date.UTC(yr, 0, 1))),
				dlY: 	dlY || (dlY = new Date(Date.UTC(yr+1, 0, 0))),
				wOY:    wOY || (wOY = Math.ceil((dOY - dOW)/ DAYS_PER_WEEK)),
				mt: 	mt  || (mt = date.getUTCMonth()),
				dOY: 	dOY || (dOY = Math.ceil((date - dfY + 1)/MS_PER_DAY)),				
				ldY:    ldY || (ldY = Math.ceil((dlY - dfY)/MS_PER_DAY)),
				dOM: 	dOM || (dOM = date.getUTCDate()),				
				dOW: 	dOW || (dOW = date.getUTCDay()),				
				hr: 	hr  || (hr = date.getUTCHours()),				
				mn: 	mn  || (mn = date.getUTCMinutes()),				
				sc: 	sc  || (sc = date.getUTCSeconds()),
				ti: 	ti  || (ti = pad(hr) + ':' + pad(mn) + ':' + pad(sc)),

				skipTo:  function(year, month, date, hour, min, sec) {
					date.setTime(Date.UTC(year, month, date, hour, min, sec));
					clear();
				}

				next: function(resolution) {
					date.setTime(date.getTime() + (resolution * 1000));
					clear();
				}
			}
		}
		...
		return {
			...
			getNext: function (schedule, count, startDate) {
				var start = _date(startDate)
				  , s = schedule.length ? schedule : Array(schedule)
				  , num = count || 1
				  , occurrences = []
				  , date;				
				 
				console.log(start.yr);
				return;
