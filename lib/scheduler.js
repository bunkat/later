...
(function() {
	...
	var Scheduler = function(resolution, offset) {
		...
		return {
			...
			isValid: function (schedule, date, exceptions) {
				var next = this.getNext(schedule, 1, date, exceptions);
				return next.getTime() === date.getTime();
			},

			getNext: function (schedule, count, startDate, except) {
				...
				while (num) {
					...
					for( i = 0; i < length; i++ ) {
						tDate = getNextForSchedule(s[i], start);
						if (!date || tDate < date) {
							date = tDate;
						}
					}
					...
					if (!except || !this.isValid(except, tDate)) {
