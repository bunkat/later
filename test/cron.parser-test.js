...
describe('CronParser', function() {
	...
	describe('day of month', function() {
		...
		it('should parse a single nearest first weekday', function() {
			...
			p.schedules[0].should.have.ownProperty('D').and.eql({D: [1, 2, 3], d:[1,2,3,4,5]});
			p.exceptions[0].should.eql({D: [2], d:[2,3,4,5]});
			p.exceptions[1].should.eql({D: [3], d:[2,3,4,5]});
		});

		it('should parse a single nearest weekday', function() {
			var p = parser().parse('* * * 15W * *', true);
			p.schedules[0].should.have.ownProperty('D').and.eql({D: [14, 15, 16], d:[1,2,3,4,5]});
			p.exceptions[0].should.eql({D: [14], d:[1,2,3,4]});
			p.exceptions[1].should.eql({D: [16], d:[2,3,4,5]});
		});

		it('should parse multiple single nearest weekday', function() {
			var p = parser().parse('* * * 4W,15W * *', true);
			p.schedules[0].should.have.ownProperty('D').and.eql({D: [3, 4, 5, 14, 15, 16], d:[1,2,3,4,5]});
			p.exceptions[0].should.eql({D: [3], d:[1,2,3,4]});
			p.exceptions[1].should.eql({D: [5], d:[2,3,4,5]});
			p.exceptions[2].should.eql({D: [14], d:[1,2,3,4]});
			p.exceptions[3].should.eql({D: [16], d:[2,3,4,5]});
		});
	...
	describe('months', function() {
		...
		it('should parse a single value', function() {
			var p = parser().parse('* * * * 1 *', true);
			p.schedules[0].should.have.ownProperty('M').and.eql({M: [0]});
		});

		it('should parse multiple values', function() {
			var p = parser().parse('* * * * 1,5,10 *', true);
			p.schedules[0].should.have.ownProperty('M').and.eql({M: [0,4,9]});
		});

		it('should parse a range value', function() {
			var p = parser().parse('* * * * 1-5 *', true);
			p.schedules[0].should.have.ownProperty('M').and.eql({M: [0,1,2,3,4]});
		});

		it('should parse a range with increment value', function() {
			var p = parser().parse('* * * * 1-6/2 *', true);
			p.schedules[0].should.have.ownProperty('M').and.eql({M: [0,2,4]});
		});

		it('should parse an asterisk with increment value', function() {
			var p = parser().parse('* * * * */5 *', true);
			p.schedules[0].should.have.ownProperty('M').and.eql({M: [0,5,10]});
		});
	...
	describe('day of week', function() {
		...
		it('should parse multiple values', function() {
			...
			p.schedules[0].should.have.ownProperty('d').and.eql({d: [1,2,5]});
		});
		...
		it('should parse an asterisk with increment value', function() {
			...
			p.schedules[0].should.have.ownProperty('d').and.eql({d: [0,3,6]});
		});

		it('should parse last', function() {
			var p = parser().parse('* * * * * 5L', true);
			p.schedules[0].should.have.ownProperty('d').and.eql({d: [5], dc: [-1]});
		});

		it('should parse last in combination', function() {
			var p = parser().parse('* * * * * 4,5L', true);
			p.schedules[0].should.have.ownProperty('d').and.eql({d: [4]});
			p.schedules[1].should.have.ownProperty('d').and.eql({d: [5], dc: [-1]});
		});
